...
import { newIncident } from '../api/incidents';
...
export default function NewIncidentScreen() {
  ...
  const save = async (data: FormData) => {
    ...
    try {
      const newIncidentData = await newIncident(authState.token, data.name, data.evidence, data.criticality, data.host);
      ...
      setError("Failed to create incident. Please try again later.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <View className="bg-red-500 px-6 py-6 flex flex-row items-center justify-between">
          <View className='flex flex-row items-center'>
            ...
            <View className="flex flex-col gap-y-1">
              ...
              <Text className="text-white text-2xl font-bold">New Incident</Text>
            </View>
          </View>
